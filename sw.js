   ...
   e.respondWith(
      caches.match(e.request).then(cacheResponse => {
         const fetchPromise = fetch(e.request).then(async res => {
            const cache = await caches.open(buildFiles.includes(file) ? 'build' : 'runtime')
